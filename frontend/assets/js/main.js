...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try{
    const home = await fetch('http://localhost:8080/api/home').then(r=>r.json());
    const sections = [
    ...
    sections.forEach(sec=>{
      ...
      root.innerHTML = '';
      sec.list.forEach(p=>{
        const price = window.pricing.formatPair(window.pricing.computePriceUSD(p, {}));
        const el = document.createElement('a');
        el.className='card';
        el.href = `./product.html?id=${encodeURIComponent(p.id)}`;
        el.innerHTML = `<div><strong>${p.title}</strong></div>
                        <div class="price">${price.current} ${price.old? `<span class='old'>${price.old}</span>`:''}</div>
  ...
  if(document.getElementById('storesSlider')){
    const stores = await fetch('http://localhost:8080/api/stores').then(r=>r.json()).catch(()=>[]);
    const products = await fetch('http://localhost:8080/api/products').then(r=>r.json()).catch(()=>[]);
    const wrap = document.getElementById('storesSlider');
    wrap.innerHTML='';
    stores.forEach(s=>{
      const slide = document.createElement('div');
      slide.className = 'store-slide';
      slide.innerHTML = `<div class="store-head"><div class="logo">${s.logo||'🏬'}</div><div><h3>${s.name}</h3></div></div>
                         <div class="carousel row"></div>`;
      const row = slide.querySelector('.row');
      products.filter(p => p.storeId === s.id).forEach(p => {
        const price = window.pricing.formatPair(window.pricing.computePriceUSD(p, {}));
  ...
  if(document.getElementById('productView')){
    ...
    const p = await fetch(`http://localhost:8080/api/products/${id}`).then(r=>r.json());
    const root = document.getElementById('productView');
    const price = window.pricing.formatPair(window.pricing.computePriceUSD(p, {}));
    // Variants selector
    const options = (p.variants||[]).map(v => `<option value="${v.id}" ${v.stock<=0?'disabled':''}>${v.size} — stock: ${v.stock}</option>`).join('');
