...
(async function () {
  ...
  if (path.endsWith("/admin.html")) {
    // Rates
    const r = await fetch("http://localhost:9090/api/config/rates")
      .then((r) => r.json())
    ...
    document.getElementById("saveRates").onclick = async () => {
      ...
      await fetch("http://localhost:9090/api/config/rates", {
        method: "POST",
    ...
    // Shipping
    const s = await fetch("http://localhost:9090/api/config/shipping")
      .then((r) => r.json())
    ...
    document.getElementById("saveShipping").onclick = async () => {
      ...
      await fetch("http://localhost:9090/api/config/shipping", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
